...
#' lz77_RLE_compressor. Encodes the string with RLE compressor followed by lz77 compressor (either list of triplet or vector)
#'
#' @param  vector of characters
#' @return l_s list of triplets (offset, length, symbol) or vector of characters
#' @export
#' @examples
#' test_lz <- "abacabacabadaca"

lz77_RLE_compressor <- function(s) {
  lz77_compressor(RLE_compressor(s, as.vec = T))
}

#' RLE_lz77_compressor. Encodes the string with LZ77 compressor followed by RLE compressor (either list of triplet or vector)
#'
#' @param l_s list of triplets (offset, length, symbol) or vector of characters
#' @return vector of characters
#' @export
#' @examples
#' test_lz <- "abacabacabadaca"
RLE_lz77_compressor <- function(s) {
  RLE_compressor(lz77_compressor(s), as.vec=T)
}

#' lz77_RLE_decompressor. decodes the string with lz77 decompressor followed by RLE compressor (either list of triplet or vector)
#'
#' @param l_s list of triplets (offset, length, symbol) or vector of characters
#' @return vector of characters
#' @export
...
lz77_RLE_decompressor <- function(s) {
  RLE_decompressor(lz77_decompressor(s), as.vec = T)
}

#' RLE_lz77_decompressor. decodes the string with RLE decompressor followed by LZ77 compressor (either list of triplet or vector)
#'
#' @param vector of characters
#' @return l_s list of triplets (offset, length, symbol) or vector of characters
#' @export
#' @examples
#' test_lz <- "abacabacabadaca"
RLE_lz77_decompressor <- function(s) {
  lz77_decompressor(RLE_decompressor(s, as.vec = F))
}
